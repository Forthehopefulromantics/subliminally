...
/* The ladder, as TIER_ORDER in js/billing.js has it: free, then Ritual, and
   nothing between them. Server-side so a route can ask "is this plan enough for
   that feature" the same way the page does, rather than comparing strings. */
const TIER_ORDER = ['none', 'ritual'];
export function tierAtLeast(myTier, requiredTier) {
  const rank = (t) => TIER_ORDER.indexOf(LEGACY_TIER_TO_TIER[t] || t || 'none');
  return rank(myTier) >= rank(requiredTier);
}
