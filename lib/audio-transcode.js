...
/* Why a real recording can measure as "silent" without the mic being muted:
   a phone mic that recorded actual speech, but quietly — a bit of distance
   from the mic, a soft voice, or (on iOS specifically) autoGainControl simply
   not being applied the way it is on a call, because turning off echo
   cancellation and noise suppression for a cleaner sample also turns off the
   gain stage that rides along with them. The bytes are real audio, just at a
   peak level the fixed loudness check was never going to pass.

   dynaudnorm brings the quiet parts of real speech up to a normal level while
   leaving true silence alone (there is nothing in it to bring up) — the fix,
   not a looser threshold. Run on every sample this route accepts, WAV
   passed straight through by the page included, so the check downstream sees
   the same loudness a real recording should have had. */
export async function normalizeLoudness(wavBuf) {
  if (!ffmpegPath) throw new TranscodeError('transcode_unavailable', 'no ffmpeg binary for this platform');
  const dir = await mkdtemp(join(tmpdir(), 'voice-norm-'));
  const input = join(dir, 'in.wav');
  const output = join(dir, 'out.wav');
  try {
    await writeFile(input, wavBuf);
    await runFfmpeg([
      '-hide_banner', '-nostdin', '-loglevel', 'error', '-y',
      '-i', input,
      // f: analysis window (ms); g: gain-smoothing window (frames); p: target peak (0..1);
      // m: max gain the filter may apply — capped so background hiss in a truly quiet
      // room cannot be amplified into something that reads as speech.
      '-af', 'dynaudnorm=f=250:g=15:p=0.85:m=12',
      '-ac', '1', '-ar', String(SAMPLE_RATE), '-c:a', 'pcm_s16le',
      '-fflags', '+bitexact', '-flags:a', '+bitexact',
      '-f', 'wav', output,
    ]);
