...
/* The two things somebody has to agree to before a voice of theirs is made.
   They are separate on purpose: one is about whose voice it is, the other about
   where the recording goes. Agreeing to the first is not agreeing to the second.

   Both live here so the page cannot show one thing and the server accept
   another: /api/voices hands these exact strings to the page, and
   /api/voice-clone refuses a sample that does not come back with both. */
export const VOICE_CLONE_CONSENT =
  'I confirm this is my voice and I have permission to create an AI voice clone from it.';

export const VOICE_CLONE_PROCESSING_CONSENT =
  'I consent to my recording being sent to ElevenLabs and processed there to create my AI voice.';

export function consentGiven(headerValue) {
  return typeof headerValue === 'string' && headerValue.trim() === VOICE_CLONE_CONSENT;
}

export function processingConsentGiven(headerValue) {
  return typeof headerValue === 'string' && headerValue.trim() === VOICE_CLONE_PROCESSING_CONSENT;
}
