...
/* Instant Voice Cloning: one sample in, one voice id out. */
export async function createInstantVoiceClone({ name, description, sample, contentType, fileName }) {
  ...
  const res = await callProvider('/voices/add', { method: 'POST', body: form }, CLONE_TIMEOUT_MS);
  const created = await res.json().catch(() => null);
  if (!created || !created.voice_id) throw new VoiceProviderError('rejected', 'no voice_id in response');
