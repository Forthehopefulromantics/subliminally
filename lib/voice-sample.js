...
//
// Every sample is checked as 16-bit PCM WAV, mono — what ElevenLabs reads
// without question. The page converts recordings and uploads to it before they
// leave the device; anything that arrives in another format (M4A, MP4, MOV, MP3,
// WebM, …) is transcoded first by lib/audio-transcode.js. So the length check
// below is on the audio itself, not on a timer the page ran.
...
// Below this, on the audio as it actually arrived (before any loudness
// normalization), there is nothing there to bring up — a muted or disconnected
// mic, not a quiet voice. Real speech, however soft, has consonants and
// breath that peak well above this; a dead input does not. This is checked
// before normalization runs, so a boosted noise floor can never pass as
// speech (see lib/audio-transcode.js normalizeLoudness).
