...
function showStep(n){
  ...
  // Which of the three voice cards is lit, and whether the middle one still says
  // "Clone Your Voice" — both depend on what this account has, so they are drawn
  // on arrival rather than only when something is tapped.
...
/* ---------------- step 4: voice choice ----------------
   Three answers, and only three:
     ...
     Serenity              the built-in voice. Generated by /api/tts through
                           ElevenLabs, free account or paid.
     Clone Your Voice      an AI voice made once from a sample of theirs, which
                           can then read anything they write later. Ritual only.
     Record Your Own Voice they read the affirmations themselves and that audio
                           is what plays.

   The last two are not the same thing and the code never treats them as one: a
   recording is the person's own audio, stays their own audio, and is never sent
   to ElevenLabs or turned into a clone.

   When /api/tts isn't configured — or a generated voice fails for any reason —
   the app falls back to the device's own speech synthesis, which is how it has
   always worked, so nothing breaks. */
/* What the page holds is a *key* — 'serenity', 'mine', 'device' — and never a
   ...
   The name comes from /api/voices for anybody signed in. The constant below is
   only the fallback for a visitor who has not signed in yet, or a catalogue
   request that did not land — a name, because without an account there is
   nothing to generate with anyway and every preview is read by the device. */
const SERENITY_VOICE = 'serenity';
const FALLBACK_PRESET_VOICES = [
  { key: SERENITY_VOICE, name: 'Serenity', desc: 'Calm, warm AI voice' },
];
...
/* Shown if the catalogue cannot be reached. The server holds the real ones and
...
const VOICE_PROCESSING_CONSENT_FALLBACK = 'I consent to my recording being sent to ElevenLabs and processed there to create my AI voice.';

let voiceCatalogue = {
  presets: FALLBACK_PRESET_VOICES,
  myVoice: null,
  consentStatement: VOICE_CONSENT_FALLBACK,
  processingConsentStatement: VOICE_PROCESSING_CONSENT_FALLBACK,
  provider: null,
  loaded: false,
};
  ...
  voiceCataloguePromise = (async () => {
    ...
    try {
      ...
      voiceCatalogue = {
        ...
        processingConsentStatement: out.processingConsentStatement || VOICE_PROCESSING_CONSENT_FALLBACK,
...
async function voiceProcessingConsentStatement(){ return (await loadVoiceCatalogue()).processingConsentStatement; }
/* What Serenity is called on the card, once the catalogue has been asked. */
function serenityVoiceName(){
  const preset = (voiceCatalogue.presets || []).find(v => v.key === SERENITY_VOICE);
  return (preset && preset.name) || 'Serenity';
}

/* The voices the picker used to offer. A subliminal saved when there were six of
   them names one of these, and the server resolves it to Serenity — so the card
   that lights up when it is reopened should be Serenity too, rather than none of
   them. lib/voices.js has the same list; the migration rewrites the rows. */
const RETIRED_VOICE_KEYS = [
  'sarah', 'charlotte', 'alice', 'lily', 'daniel', 'george',
...
function normalizeVoiceKey(key){
  if (!key) return null;
  if (RETIRED_VOICE_KEYS.indexOf(key) !== -1) return SERENITY_VOICE;
  return key;
}
...
/* ---------------- the three answers ----------------
   'serenity' — the built-in voice reads the lines (ElevenLabs, server-side).
   'clone'    — the AI voice made from a sample of this person's voice reads them.
   'own'      — this person records themselves reading them, and that recording
                is what plays. It is never sent to ElevenLabs and never becomes a
                clone: different thing, different button.

   Underneath, the first two are both state.voiceMode === 'ai' with a different
   voice key, which is what everything downstream — saving, reopening, the
   player — has always understood. Nothing below this function had to change. */
function chooseVoice(mode){
  if (mode === 'clone'){ chooseClonedVoice(); return; }
  ...
  if (mode === 'serenity') previewVoice(SERENITY_VOICE);
  advanceAfterFirstVoicePick();
}

/* Answering moves you on — but only the first time. Tapping Serenity a second
   time is somebody listening to it again before they commit, and throwing them
   forward a screen for that would punish the one thing the preview is for. */
let voicePickMoved = false;
function advanceAfterFirstVoicePick(){
  if (voicePickMoved) return;
  voicePickMoved = true;
  advanceAfterPick();
}

/* "Clone Your Voice" is the one card that is not simply a pick. It is a plan
   check, and then either the voice they already have — chosen, previewed, done —
   or the onboarding panel that explains what cloning is and asks for the two
   confirmations before anything is recorded. */
async function chooseClonedVoice(){
  ...
  if (!mine){
    ...
    if (!tierHasFeature(tier, 'my_voice')){
      // The existing paywall, not a new one, and nothing is recorded or sent.
  ...
  renderVoiceCards();
  document.getElementById('toStep5').disabled = false;
  previewVoice(MY_CLONED_VOICE);
  advanceAfterFirstVoicePick();
}

/* Which of the three cards is lit, and what the middle one is called. Once a
   voice has been cloned it stops asking ("Clone Your Voice") and starts stating
   ("Your AI Voice ✓"), because by then it is a voice they have rather than a
   thing they might do. */
async function renderVoiceCards(){
  const cloneCard = document.getElementById('voiceClone');
  if (!cloneCard) return;
  const cat = await loadVoiceCatalogue();
  const tier = currentUser ? await getMyTier() : 'none';

  const picked = state.voiceMode === 'own'
    ? 'own'
    : (normalizeVoiceKey(state.aiVoiceId) === MY_CLONED_VOICE ? 'clone'
      : (state.voiceMode === 'ai' ? 'serenity' : null));

  const serenityCard = document.getElementById('voiceSerenity');
  const ownCard = document.getElementById('voiceOwn');
  if (serenityCard){
    serenityCard.classList.toggle('sel', picked === 'serenity');
    const h = serenityCard.querySelector('h4');
    if (h) h.textContent = serenityVoiceName();
  }
  if (ownCard) ownCard.classList.toggle('sel', picked === 'own');
  cloneCard.classList.toggle('sel', picked === 'clone');

  const title = document.getElementById('voiceCloneTitle');
  const desc = document.getElementById('voiceCloneDesc');
  if (title) title.textContent = cat.myVoice ? 'Your AI Voice ✓' : 'Clone Your Voice';
  if (desc) desc.textContent = cat.myVoice
    ? 'The AI voice made from your own — manage it in Settings'
    : 'Create an AI version of your own voice';

  /* The lock and the RITUAL badge come off the same feature table as the rest of
     the app — nothing is removed from a free account's screen, it is drawn with
     a lock and explains itself when tapped. Only while there is no voice yet:
     once somebody has one it is theirs, whatever happens to their plan later. */
  if (cat.myVoice){
    cloneCard.classList.remove('is-locked');
    const tag = cloneCard.querySelector('.locked-tag');
    if (tag) tag.remove();
  } else {
    lockedControl(cloneCard, 'my_voice', tier);
  }
...
function renderMyVoicePanel(){
  ...
  panel.innerHTML = `
    <div class="voice-clone-card">
      <h4 class="voice-clone-title">Create Your Voice</h4>
      <p class="voice-clone-intro">Read the passage below once, in a quiet room. That one clean sample is sent securely to ElevenLabs, our voice provider, and used to create a personal AI version of your voice for your account — after that every subliminal you build can be read in it, without recording line by line.</p>
      <p class="voice-clone-intro">You will hear the recording back and can record it again before anything is sent. The recording itself is never kept: only the voice it creates.</p>
...
/* Called by the recorder once a voice exists. The person asked for their voice by
   tapping the card, so the card is what they get back — now reading "Your AI
   Voice ✓", selected, panel closed — rather than a message telling them to tap
   it again. */
...
async function resolveVoiceKey(picked){
  const key = normalizeVoiceKey(picked);
  if (!key || key === DEVICE_VOICE) return null;
  if (!currentUser) return null;
  if (key === MY_CLONED_VOICE) return (await myVoiceProfile()) ? MY_CLONED_VOICE : null;
  return key;
}
...
const TTS_MESSAGES = {
  not_signed_in:    'Sign in to hear Serenity — this device reads it for now.',
  upgrade_required: 'Your AI voice comes with Ritual — Serenity can read it in the meantime.',
  not_configured:   'Serenity isn\'t switched on yet — using this device\'s voice.',
  no_cloned_voice:  'Your AI voice hasn\'t been created yet — pick "Clone Your Voice" to read the passage once.',
  invalid_voice:    'That voice isn\'t available any more — choose another one, or this device reads it.',
  consent_required: 'Tick both confirmations first, then your voice can be created.',
  processing_consent_required: 'Tick the second confirmation too — the recording has to go to ElevenLabs for the voice to be made.',
  quota_exceeded:   'The voice service has run out of time this month — this device reads it for now, and it\'ll be back.',
  ...
  hourly_limit:     'You\'ve built a lot this hour. The generated voices come back shortly; this device reads it until then.',
  daily_limit:      'You\'ve built a lot today. The generated voices come back tomorrow; this device reads it until then.',
...
function finishFinal(){
  ...
  /* THE ECHO. A pass ends with timers still in flight: the gap after the last
     line, the layered voice's own loop, a speech watchdog. finalPlaying going
     false makes each of them a no-op -- but only while it stays false. Press
     play again within that gap (and the gap is over two seconds) and the stale
     callback woke up inside the NEW session and carried on from where the old
     one stopped, reading the same lines a beat behind the new pass. Two voice
     tracks, slightly offset, from one recording: the doubling people heard.

     stopFinal() has always cleared these. finishFinal did not, and it is the
     one that runs every time a session ends by itself, which is most of them. */
