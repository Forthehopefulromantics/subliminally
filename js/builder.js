...
/* ---------------- step 4: voice choice ----------------
   ...
   not — which voice reads the lines.

   Recording yourself is free, on every account, and never touches ElevenLabs.
   Serenity and a cloned voice are premium, and both are generated by /api/tts;
   when that isn't configured (or the person isn't on a paid plan) the app falls
   back to the device's own speech synthesis, which is how it has always worked,
   so nothing breaks. */
/* What the picker offers is a *key* — 'serenity', 'mine', 'device' — and never a
   ...
   V1 offers one AI voice, Serenity, and it is premium. The six voices this
   picker used to list are retired: they are not offered here and /api/voices no
   longer returns them, but the server still resolves them so a subliminal saved
   with one keeps playing. See LEGACY_PRESET_VOICES in lib/voices.js.
...
async function renderVoiceChips(){
  ...
  const tier = currentUser ? await getMyTier() : 'none';
  const canStudio = tierHasFeature(tier, 'studio_voice');

  /* Nothing premium is pre-selected for somebody who cannot use it: a free
     account lands on the device voice, which is the one here that works for
     them, rather than on a locked chip that fails the moment they continue. */
  if (!state.aiVoiceId) state.aiVoiceId = canStudio ? cat.presets[0].key : DEVICE_VOICE;

  /* Serenity, then "Use my voice", then the device — and all three are always
     drawn, locked or not. That is the same rule as every other premium control
     in the app: nothing is removed from a free account's screen, it is drawn
     with a lock and explains itself when tapped. */
  const chips = cat.presets.map(v => ({ key: v.key, name: v.name, desc: v.desc, mine: false }));
  chips.push({
    key: MY_CLONED_VOICE,
    name: cat.myVoice ? (cat.myVoice.name || 'My voice') : 'Clone your voice',
    desc: cat.myVoice ? 'your voice' : 'an AI version of your voice',
    mine: true,
  });
  ...
  cat.presets.forEach(v => lockedControl(wrap.querySelector(`[data-voice-key="${v.key}"]`), 'studio_voice', tier));
...
async function pickAiVoice(key){
  ...
  /* Serenity is premium. A free account gets the paywall and nothing else —
     no preview, no /api/tts call, so no ElevenLabs request and no spend. The
     server refuses it too (see the gate at the top of /api/tts); this is the
     half that makes the refusal feel like an offer instead of an error. */
  if (key !== MY_CLONED_VOICE && key !== DEVICE_VOICE){
    if (!currentUser){
      if (msg){ msg.textContent = 'Create a free account first, then Serenity can read your affirmations.'; msg.className = 'save-msg err'; }
      return;
    }
    const tier = await getMyTier();
    if (!tierHasFeature(tier, 'studio_voice')){
      openUpgradeModal('studio_voice', { tier, trigger: 'voice_chip' });
      return;
    }
  }
